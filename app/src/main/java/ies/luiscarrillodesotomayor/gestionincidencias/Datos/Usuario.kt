...
open class Usuario {
    ...
    // Inicio de sesión
    /*
    fun iniciarSesion(usuario: Usuario): Boolean {
        var logeado = false
    ...
    */
    ...
    // Obtener el nombre de usuario por su email

    /*
    fun getNombreUsuario(email: String): String {
        var nombre = ""
        val database = Firebase.database
        val myRef = database.getReference("Usuarios")
        myRef.get().addOnSuccessListener {
            if (it.hasChild(email)) {
                nombre = it.child(email).child("nombre").value.toString()
            }
        }
        return nombre
    }
